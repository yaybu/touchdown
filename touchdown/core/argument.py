...
class Argument(object):
    ...
    def register_adapter(cls, klass, callable):
        cls.adapters.setdefault(cls, []).append((klass, callable))
...
class String(Argument):
    ...
    def clean(self, instance, value):
        ...
        if self.choices is not None and value not in self.choices:
            raise errors.InvalidParameter("Not a valid value for this field")
...
class IPNetwork(String):
    ...
    def clean(self, instance, value):
        ...
        if value != str(network.cidr):
            raise errors.InvalidParameter(
                "{} looks wrong - did you mean {}?".format(value, network.cidr)
            )
        return network
...
class Resource(Argument):
    ...
    def get_resource_class(self):
        ...
        return tuple(
            itertools.chain(
                self.resource_class, *[r.__subclasses__() for r in self.resource_class]
            )
        )
        ...
        if hasattr(self.resource_class, "wrap"):
            ...
            for resource_class in self.get_resource_class():
                try:
                    ...
                    error_msg.append(
                        " * {}: {}".format(resource_class.resource_name, str(e))
                    )
            else:
                error_msg.insert(
                    0,
                    "Parameter is not the right type and could not be converted into the right type:",
                )
                raise errors.InvalidParameter("\n".join(error_msg))
    ...
    def contribute_to_class(self, cls):
        ...
        if isinstance(self.resource_class, six.string_types):
            ...
            if self.resource_class not in ResourceType.__all_resources__:
                ResourceType.add_callback(
                    self.resource_class, self.contribute_to_class, cls
                )
                return
...
class ResourceList(List):
    def __init__(self, resource_class, **kwargs):
        super(ResourceList, self).__init__(Resource(resource_class), **kwargs)
