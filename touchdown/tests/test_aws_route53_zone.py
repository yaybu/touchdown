...
class TestHostedZoneCreation(StubberTestCase):
    def test_create_hosted_zone(self):
        ...
        zone = self.fixtures.enter_context(
            HostedZoneStubber(
                goal.get_service(self.aws.add_hosted_zone(name="example.com"), "apply")
            )
        )
...
class TestUpdateRrset(StubberTestCase):
    ...
    def test_add_rrset_alias_loadbalancer(self):
        ...
        lb = self.fixtures.enter_context(
            LoadBalancerStubber(
                goal.get_service(
                    self.aws.get_load_balancer(name="my-load-balancer"), "describe"
                )
            )
        )
    ...
    def test_add_rrset_alias_distribution(self):
        ...
        distribution = self.fixtures.enter_context(
            DistributionStubber(
                goal.get_service(
                    self.aws.get_distribution(name="www.example.com"), "describe"
                )
            )
        )
    ...
    def test_delete_rrset(self):
        ...
        zone = self.fixtures.enter_context(
            HostedZoneStubber(
                goal.get_service(
                    self.aws.add_hosted_zone(name="example.com", records=[]), "apply"
                )
            )
        )
...
class TestHostedZoneDeletion(StubberTestCase):
    def test_delete_hosted_zone(self):
        ...
        zone = self.fixtures.enter_context(
            HostedZoneStubber(
                goal.get_service(
                    self.aws.add_hosted_zone(name="example.com"), "destroy"
                )
            )
        )
