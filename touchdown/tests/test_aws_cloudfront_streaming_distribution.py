...
class TestStreamingDistributionCreation(aws.StubberTestCase):
    def test_create_streaming_distribution(self):
        ...
        streaming_distribution = self.fixtures.enter_context(
            StreamingDistributionStubber(
                goal.get_service(
                    self.aws.add_streaming_distribution(
                        name="www.example.com", bucket=bucket.bucket
                    ),
                    "apply",
                )
            )
        ...
        streaming_distribution.add_create_streaming_distribution(
            EXAMPLE_DISTRIBUTION_CONFIG
        )

        streaming_distribution.add_list_streaming_distributions_one_response(
            EXAMPLE_DISTRIBUTION_CONFIG, status="InProgress"
        )
        streaming_distribution.add_get_streaming_distribution(
            EXAMPLE_DISTRIBUTION_CONFIG, status="InProgress"
        )
        streaming_distribution.add_get_streaming_distribution(
            EXAMPLE_DISTRIBUTION_CONFIG, status="Deployed"
        )
...
class TestStreamingDistributionDeletion(aws.StubberTestCase):
    def test_delete_streaming_distribution(self):
        ...
        streaming_distribution.add_update_streaming_distribution(
            streaming_distribution.disable(EXAMPLE_DISTRIBUTION_CONFIG)
        )
    ...
    def test_delete_streaming_distribution_idempotent(self):
        ...
        streaming_distribution = self.fixtures.enter_context(
            StreamingDistributionStubber(
                goal.get_service(
                    self.aws.add_streaming_distribution(name="www.example.com"),
                    "destroy",
                )
            )
        )
