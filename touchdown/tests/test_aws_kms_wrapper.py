...
class TestKmsWrapper(StubberTestCase):
    ...
    def test_edit_file_encrypted_with_kms(self):
        ...
        key = self.fixtures.enter_context(
            KeyStubber(goal.get_service(self.aws.add_key(name="test-key"), "describe"))
        )
        ...
        wrapped_file = key.resource.add_cipher(
            name="test.txt", file=folder.add_file(name="test.txt.kms")
        )
    ...
    def test_inifile_string_encrypted_with_kms(self):
        ...
        variable = config_file.add_string(name="strings.variable1", default="value1")
    ...
    def test_inifile_string_encrypted_with_kms_on_s3(self):
        ...
        folder = self.fixtures.enter_context(
            BucketStubber(
                goal.get_service(self.aws.get_bucket(name="test-bucket"), "describe")
            )
        )
        ...
        file = self.fixtures.enter_context(
            BucketStubber(
                goal.get_service(folder.resource.add_file(name="test.cfg"), "describe")
            )
        )
        ...
        file.add_response(
            "get_object",
            service_response={
                "Body": StreamingBody(six.BytesIO(old_value), len(old_value))
            },
            expected_params={"Bucket": "test-bucket", "Key": "test.cfg"},
        )

        file.add_response(
            "put_object",
            service_response={},
            expected_params={"Body": ANY, "Bucket": "test-bucket", "Key": "test.cfg"},
        )
        ...
        file.add_response(
            "get_object",
            service_response={
                "Body": StreamingBody(six.BytesIO(new_value), len(new_value))
            },
            expected_params={"Bucket": "test-bucket", "Key": "test.cfg"},
        )
