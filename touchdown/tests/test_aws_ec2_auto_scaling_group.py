...
class TestCreateAutoScalingGroupuration(StubberTestCase):
    def test_create_auto_scaling_group(self):
        ...
        launch_config = self.fixtures.enter_context(
            LaunchConfigurationFixture(goal, self.aws)
        )

        auto_scaling_group = self.fixtures.enter_context(
    ...
    def test_create_auto_scaling_group_wait_for_healthy(self):
        ...
        auto_scaling_group.add_describe_auto_scaling_groups_empty_response()
        auto_scaling_group.add_create_auto_scaling_group(min_size=1, max_size=1)
        auto_scaling_group.add_describe_auto_scaling_groups_one_response()
        auto_scaling_group.add_describe_auto_scaling_groups_one_response()
        auto_scaling_group.add_describe_auto_scaling_groups_one_response()
    ...
    def test_destroy_auto_scaling_group(self):
        ...
        auto_scaling_group = self.fixtures.enter_context(
            AutoScalingGroupStubber(
                goal.get_service(
                    self.aws.add_auto_scaling_group(
                        name="my-asg", min_size=0, max_size=0
                    ),
                    "destroy",
                )
            )
...
class TestSshAutoScalingGroup(StubberTestCase):
    def test_ssh_auto_scaling_group_no_instances(self):
        ...
        auto_scaling_group = self.fixtures.enter_context(
            AutoScalingGroupStubber(
                goal.get_service(
                    self.aws.get_auto_scaling_group(name="my-asg"), "describe"
                )
            )
        )
        auto_scaling_group.add_describe_auto_scaling_groups_one_response(instances=[])
    ...
    def test_ssh_auto_scaling_group_no_instances_in_service(self):
        ...
        auto_scaling_group.add_describe_auto_scaling_groups_one_response(
            instances=[{"LifecycleState": "Terminating"}]
        )
    ...
    def test_ssh_auto_scaling_group_instances_not_found_in_ec2(self):
        ...
        auto_scaling_group.add_describe_auto_scaling_groups_one_response(
            instances=[{"LifecycleState": "InService", "InstanceId": "i-abcde23f"}]
        )

        ec2_client_stub = self.fixtures.enter_context(
            Stubber(auto_scaling_group.service.ec2_client)
        )
        ec2_client_stub.add_response(
            "describe_instances",
            service_response={"Reservations": []},
            expected_params={"InstanceIds": ["i-abcde23f"]},
        )
    ...
    def test_ssh_auto_scaling_group_direct(self):
        ...
        ec2_client_stub.add_response(
            "describe_instances",
            service_response={
                "Reservations": [{"Instances": [{"PublicIpAddress": "8.8.8.8"}]}]
            },
            expected_params={"InstanceIds": ["i-abcde23f"]},
        )
        ...
        self.assertEqual(
            ssh_connection.get_command_and_args(),
            [
                "/usr/bin/ssh",
                "-o",
                'User="root"',
    ...
    def test_ssh_auto_scaling_group_via_auto_scaling_group(self):
        ...
        ec2_client_stub.add_response(
            ...
            service_response={
                "Reservations": [
                    {
                        "Instances": [
                            {"PublicIpAddress": "8.8.8.8", "VpcId": "vpc-de3db33"}
                        ]
                    }
                ]
            },
        ...
        auto_scaling_group2 = self.fixtures.enter_context(
            AutoScalingGroupStubber(
                goal.get_service(
                    self.aws.get_auto_scaling_group(name="my-asg-2"), "describe"
                )
            )
        )
        auto_scaling_group2.add_describe_auto_scaling_groups_one_response(
            instances=[{"LifecycleState": "InService", "InstanceId": "i-abcde23g"}]
        )

        ec2_client_stub2 = self.fixtures.enter_context(
            Stubber(auto_scaling_group2.service.ec2_client)
        )
        ec2_client_stub2.add_response(
            ...
            service_response={
                "Reservations": [
                    {
                        "Instances": [
                            {"PrivateIpAddress": "10.0.0.1", "VpcId": "vpc-de3db33"}
                        ]
                    }
                ]
            },
            expected_params={"InstanceIds": ["i-abcde23g"]},
        )
        ...
        self.assertEqual(
            ssh_connection_2.get_command_and_args(),
            [
                "/usr/bin/ssh",
                "-o",
                'User="root"',
                ...
                'HostName="10.0.0.1"',
                "-o",
                'ProxyCommand=/usr/bin/ssh -o User="root" -o Port="22" -W %h:%p 8.8.8.8',
                "remote",
            ],
        )
    ...
    def test_ssh_ec2_instance_via_auto_scaling_group(self):
        ...
        ec2_instance = self.fixtures.enter_context(
            EC2InstanceStubber(
                goal.get_service(
                    self.aws.get_ec2_instance(name="my-ec2-instance"), "describe"
                )
            )
        )
