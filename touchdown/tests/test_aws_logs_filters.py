...
class TestCreateLogFilter(StubberTestCase):
    def test_create_log_filter(self):
        ...
        log_group = self.fixtures.enter_context(
            LogGroupStubber(
                goal.get_service(self.aws.add_log_group(name="test-log-group"), "apply")
            )
        )
        log_group.add_describe_log_groups_one_response()

        log_filter = self.fixtures.enter_context(
            LogFilterStubber(
                goal.get_service(
...
class TestDestroyLogFilter(StubberTestCase):
    def test_destroy_log_filter(self):
        ...
        log_group = self.fixtures.enter_context(
            LogGroupStubber(
                goal.get_service(
                    self.aws.get_log_group(name="test-log-group"), "describe"
                )
            )
        )
        log_group.add_describe_log_groups_one_response()
        ...
        log_filter = self.fixtures.enter_context(
            LogFilterStubber(
                goal.get_service(
                    log_group.resource.add_filter(name="test-log-filter"), "destroy"
                )
            )
        )
        log_filter.add_describe_log_filter_one_response()
