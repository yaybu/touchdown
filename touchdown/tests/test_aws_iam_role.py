...
class TestCreateRole(StubberTestCase):
    def test_create_role(self):
        ...
        role = self.fixtures.enter_context(
            RoleStubber(
                goal.get_service(
                    self.aws.add_role(
                        name="my-test-role",
...
class TestUpdateRole(StubberTestCase):
    ...
    def test_remove_policy(self):
        ...
        role = self.fixtures.enter_context(
            RoleStubber(
                goal.get_service(
                    self.aws.add_role(name="my-test-role", policies={}), "apply"
                )
            )
        )
...
class TestGetSigninUrl(StubberTestCase):
    def test_get_signin_url(self):
        ...
        role = self.fixtures.enter_context(
            RoleStubber(
                goal.get_service(self.aws.get_role(name="read-only"), "get-signin-url")
            )
        )
        ...
        get = self.fixtures.enter_context(
            mock.patch("touchdown.aws.iam.role.requests.get")
        )
...
class TestGetCredentials(StubberTestCase):
    def test_get_temporary_credentials(self):
        ...
        role = self.fixtures.enter_context(
            RoleStubber(
                goal.get_service(self.aws.get_role(name="read-only"), "get-credentials")
            )
        )
