...
class TestGenericAction(unittest.TestCase):
    def test_basic_call(self):
        ...
        g = common.GenericAction(
            plan, "I am an action", api, serializer=serializers.Resource()
        )
        self.assertEqual(tuple(g.description), ("I am an action",))
        g.run()

        api.assert_called_with(NumCacheNodes=1, CacheClusterId="freddy")


class TestSimpleDescribeImplementations(unittest.TestCase):

    ignore = (common.SimpleApply, common.SimpleDestroy)
