...
EXAMPLE_DISTRIBUTION_CONFIG = {
    ...
    "DefaultRootObject": "/",
    "Enabled": True,
    "Logging": {"Bucket": "", "Enabled": False, "IncludeCookies": False, "Prefix": ""},
    "Origins": {
        "Items": [
            {
...
class TestMetadata(unittest.TestCase):
    def test_waiter_waity_enough(self):
        waiter = session.get_waiter_model(
            "cloudfront", api_version=Describe.api_version
        )
...
class TestDistributionCreation(aws.StubberTestCase):
    def test_create_distribution(self):
            ...
            DistributionStubber(
                goal.get_service(
                    self.aws.add_distribution(
                        ...
                        origins=[
                            {"name": "fred", "domain_name": "backend.example.com"}
                        ],
                        default_cache_behavior={"target_origin": "fred"},
                    ),
                    "apply",
                )
            )
        ...
        distribution.add_list_distributions_one_response(
            EXAMPLE_DISTRIBUTION_CONFIG, status="InProgress"
        )
        distribution.add_get_distribution(
            EXAMPLE_DISTRIBUTION_CONFIG, status="InProgress"
        )
        distribution.add_get_distribution(
...
class TestDistributionDeletion(aws.StubberTestCase):
    def test_delete_distribution(self):
        ...
        distribution.add_update_distribution(
            distribution.disable(EXAMPLE_DISTRIBUTION_CONFIG)
        )
