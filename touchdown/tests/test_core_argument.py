...
class TestArguments(unittest.TestCase):
    ...
    def test_not_an_integer(self):
        self.assertRaises(
            errors.InvalidParameter, argument.Integer().clean, None, "five"
        )

    def test_ip_address(self):
        self.assertEqual(
            str(argument.IPAddress().clean(None, "192.168.0.1")), "192.168.0.1"
        )

    def test_ip_address_exception(self):
        self.assertRaises(
            errors.InvalidParameter, argument.IPAddress().clean, None, "192.168.0.1/24"
        )

    def test_ip_network(self):
        self.assertEqual(
            str(argument.IPNetwork().clean(None, "192.168.0.0/25")), "192.168.0.0/25"
        )

    def test_ip_network_exception(self):
        self.assertRaises(
            errors.InvalidParameter, argument.IPNetwork().clean, None, "192.168.0.270"
        )
    ...
    def test_not_a_dict(self):
        self.assertRaises(errors.InvalidParameter, argument.Dict().clean, None, [])
    ...
    def test_not_a_list(self):
        self.assertRaises(errors.InvalidParameter, argument.List().clean, None, {})
