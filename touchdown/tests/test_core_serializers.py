...
class TestSerializing(unittest.TestCase):
    ...
    def test_required(self):
        ...
        self.assertRaises(
            serializers.RequiredFieldNotPresent,
            serializer.render,
            self.runner,
            self.resource,
        )
...
class TestDiffing(unittest.TestCase):
    ...
    def test_matches_true(self):
        d = serializers.Resource().diff(
            self.runner, self.resource, {"GroupName": "test"}
        )
    ...
    def test_diffs_desc_set(self):
        ...
        d = serializers.Resource().diff(
            self.runner, self.resource, {"GroupName": "test", "Description": ""}
        )
        assert d.matches() is False
    ...
    def test_diffs_desc_unset(self):
        ...
        d = serializers.Resource().diff(
            self.runner,
            self.resource,
            {"GroupName": "test", "Description": "description"},
        )
        assert list(d.lines()) == ["description: ", "    'description' => ''"]
        assert d.matches() is False
...
class TestListOfStringsDiff(unittest.TestCase):
    def setUp(self):
        self.serializer = serializers.List(serializers.String())
