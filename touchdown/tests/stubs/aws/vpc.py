...
class VpcStubber(ServiceStubber):
    ...
    def add_describe_vpcs_one_response_by_name(self, state="available"):
        return self.add_response(
            "describe_vpcs",
            service_response={
                "Vpcs": [{"VpcId": self.make_id(self.resource.name), "State": state}]
            },
            expected_params={
                "Filters": [{"Name": "tag:Name", "Values": [self.resource.name]}]
            },
    ...
    def add_create_vpc(self):
        return self.add_response(
            "create_vpc",
            service_response={
                "Vpc": {"VpcId": self.make_id(self.resource.name), "State": "pending"}
            },
            expected_params={
    ...
    def add_delete_vpc(self):
        return self.add_response(
            "delete_vpc",
            service_response={},
            expected_params={"VpcId": self.make_id(self.resource.name)},
        )
