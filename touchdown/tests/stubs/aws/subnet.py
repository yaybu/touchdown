...
class SubnetStubber(ServiceStubber):
    ...
    def add_describe_subnets_one_response(self):
        return self.add_response(
            ...
            service_response={
                "Subnets": [
                    {"SubnetId": self.make_id(self.resource.name), "State": "available"}
                ]
            },
            expected_params={
                "Filters": [
    ...
    def add_describe_network_acls(self, network_acls=None):
        if not network_acls:
            network_acls = [{"NetworkAclId": "nacl-abcd1234"}]
        ...
        return self.add_response(
            ...
            service_response={"NetworkAcls": network_acls or []},
            expected_params={
                "Filters": [
                    {
                        "Name": "association.subnet-id",
                        "Values": [self.make_id(self.resource.name)],
                    }
                ]
            },
        )
    ...
    def add_create_subnet(self):
        return self.add_response(
            "create_subnet",
            service_response={"Subnet": {"SubnetId": self.make_id(self.resource.name)}},
            expected_params={
    ...
    def add_delete_subnet(self):
        return self.add_response(
            "delete_subnet",
            service_response={},
            expected_params={"SubnetId": self.make_id(self.resource.name)},
        )
