...
class KeyPairStubber(ServiceStubber):
    ...
    def add_describe_keypairs_empty_response_by_name(self):
        return self.add_response(
            ...
            expected_params={"KeyNames": [self.resource.name]},
        )

    def add_describe_keypairs_one_response_by_name(self):
        return self.add_response(
            "describe_key_pairs",
            service_response={"KeyPairs": [{"KeyName": self.resource.name}]},
            expected_params={"KeyNames": [self.resource.name]},
        )
    ...
    def add_import_key_pair(self, public_key):
        return self.add_response(
            "import_key_pair",
            service_response={"KeyName": self.resource.name},
            expected_params={
    ...
    def add_delete_key_pair(self):
        return self.add_response(
            "delete_key_pair",
            service_response={},
            expected_params={"KeyName": self.resource.name},
        )
