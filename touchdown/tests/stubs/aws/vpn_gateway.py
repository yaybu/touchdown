...
class VpnGatewayStubber(ServiceStubber):
    ...
    def add_describe_vpn_gateways_one_response(
        self, state="available", attachments=None
    ):
        return self.add_response(
            "describe_vpn_gateways",
            service_response={
    ...
    def add_create_vpn_gateway(self):
        return self.add_response(
            "create_vpn_gateway",
            service_response={
                "VpnGateway": {"VpnGatewayId": self.make_id(self.resource.name)}
            },
            expected_params={"Type": "ipsec.1"},
        )
    ...
    def add_delete_vpn_gateway(self):
        return self.add_response(
            "delete_vpn_gateway",
            service_response={},
            expected_params={"VpnGatewayId": self.make_id(self.resource.name)},
        )
