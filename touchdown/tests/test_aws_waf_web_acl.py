...
class TestWafWebACL(StubberTestCase):
    ...
    def test_annotate_web_acl(self):
        ...
        with stub:
            obj = describe.annotate_object({"WebACLId": "my-web-acl-id"})

        assert obj == {
            "WebACLId": "my-web-acl-id",
            "DefaultAction": {"Type": "BLOCK"},
            "Rules": [
                {"RuleId": "my-rule-id", "Priority": 10, "Action": {"Type": "ALLOW"}}
            ],
        }
    ...
    def test_create_web_acl(self):
        ...
        web_acl = self.aws.add_web_acl(
            name="my-web-acl", metric_name="mymetric", default_action="BLOCK"
        )
    ...
    def test_update_web_acl_with_active_rules(self):
        ...
        rule = self.aws.add_rule(name="my-ip-set", metric_name="my-metric-name")
        apply = goal.get_service(rule, "apply")
        apply.object = {"RuleId": "my-rule-id"}

        web_acl = self.aws.add_web_acl(
            name="my-web-acl",
            metric_name="mymetric",
            default_action="BLOCK",
            activated_rules=[{"action": "ALLOW", "priority": 10, "rule": rule}],
        )

        apply = goal.get_service(web_acl, "apply")
        apply.object = {"WebACLId": "my-web-acl-id"}
    ...
    def test_delete_web_acl(self):
        ...
        destroy.object = {
            "WebACLId": "my-web-acl-id",
            "Rules": [
                {"Priority": 10, "RuleId": "my-rule-id", "Action": {"Type": "BLOCK"}}
            ],
        }
