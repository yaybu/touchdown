...
class TestWafRule(StubberTestCase):
    ...
    def test_annotate_rule(self):
        ...
        stub.add_response(
            ...
            service_response={
                "Rule": {
                    "RuleId": "my-rule-id",
                    "Predicates": [
                        {"Negated": True, "Type": "test", "DataId": "dummy"}
                    ],
                }
            },
        )
        ...
        with stub:
            obj = describe.annotate_object({"RuleId": "my-rule-id"})

        assert obj == {
            "RuleId": "my-rule-id",
            "Predicates": [{"Negated": True, "Type": "test", "DataId": "dummy"}],
        }
    ...
    def test_update_rule_with_predicates(self):
        ...
        ip_set = self.aws.add_ip_set(name="my-ip-set", addresses=[])
        apply = goal.get_service(ip_set, "apply")
        apply.object = {"IPSetId": "my-ip-set-id"}

        rule = self.aws.add_rule(
            name="myrule", metric_name="mymetric", predicates=[{"ip_set": ip_set}]
        )
    ...
    def test_delete_rule(self):
        ...
        stub.add_response(
            "delete_rule",
            expected_params={"ChangeToken": "mychangetoken2", "RuleId": "my-rule-id"},
            service_response={"ChangeToken": "mychangetoken2"},
        )
