...
class TestVpnGatewayCreation(StubberTestCase):
    def test_create_vpn_gateway(self):
        ...
        vpn_gateway = self.fixtures.enter_context(
            VpnGatewayStubber(
                goal.get_service(
                    vpcf.vpc.add_vpn_gateway(name="test-vpn_gateway"), "apply"
                )
            )
        )
        ...
        vpn_gateway.add_describe_vpn_gateways_one_response(
            attachments=[{"VpcId": vpcf.vpc_id, "State": "attaching"}]
        )
        vpn_gateway.add_describe_vpn_gateways_one_response(
            attachments=[{"VpcId": vpcf.vpc_id, "State": "attached"}]
        )
...
class TestVpnGatewayDestroy(StubberTestCase):
    def test_destroy_vpn_gateway(self):
        ...
        vpn_gateway = self.fixtures.enter_context(
            VpnGatewayStubber(
                goal.get_service(
                    vpcf.vpc.add_vpn_gateway(name="test-vpn_gateway"), "destroy"
                )
            )
        )
        vpn_gateway.add_describe_vpn_gateways_one_response()
