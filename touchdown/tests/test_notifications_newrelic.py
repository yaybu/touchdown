...
class TestNewRelicNotifications(unittest.TestCase):
    ...
    def apply(self):
        self.apply_runner = goals.create(
            "apply", self.workspace, ConsoleFrontend(interactive=False), map=SerialMap
        )
        self.apply_runner.execute()

    def test_validation_apikey(self):
        self.workspace.add_newrelic_deployment_notification(name="foo", revision="foo")
        with mock.patch("touchdown.notifications.newrelic.requests"):
            self.assertRaises(errors.Error, self.apply)

    def test_validation_name(self):
        self.workspace.add_newrelic_deployment_notification(
            apikey="foo", revision="foo"
        )
        with mock.patch("touchdown.notifications.newrelic.requests"):
            self.assertRaises(errors.Error, self.apply)

    def test_validation_revision(self):
        self.workspace.add_newrelic_deployment_notification(apikey="foo", name="foo")
        with mock.patch("touchdown.notifications.newrelic.requests"):
            self.assertRaises(errors.Error, self.apply)

    def test_apikey_and_name_and_revision(self):
        self.workspace.add_newrelic_deployment_notification(
            apikey="foo", name="myapp", revision="myrevision"
        )
    ...
    def test_user(self):
        self.workspace.add_newrelic_deployment_notification(
            apikey="foo", name="myapp", revision="myrevision", user="duck"
        )
        with mock.patch("touchdown.notifications.newrelic.requests") as r:
            r.post.return_value.status_code = 201
            self.apply()
    ...
    def test_application_id(self):
        self.workspace.add_newrelic_deployment_notification(
            apikey="foo", name="myapp", revision="myrevision", application_id=1
        )
        with mock.patch("touchdown.notifications.newrelic.requests") as r:
            r.post.return_value.status_code = 201
            self.apply()
