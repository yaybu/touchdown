...
class TestVolumeAttachmentCreation(StubberTestCase):
    def test_create_volume_attachment(self):
        ...
        volume = self.fixtures.enter_context(
            VolumeStubber(
                goal.get_service(self.aws.get_volume(name="my-volume"), "describe")
            )
        )
        ...
        volume_attachment = self.fixtures.enter_context(
            VolumeAttachmentStubber(
                goal.get_service(
                    ec2_instance.resource.add_volume_attachment(
                        volume=volume.resource, device="/foo"
                    ),
                    "apply",
                )
            )
        ...
        volume_attachment.add_describe_attachments_one_response_by_volume_AVAILABLE()
        volume_attachment.add_describe_attachments_one_response_by_volume_INUSE(
            instance_id="i-79ebae15"
        )
