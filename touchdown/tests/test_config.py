...
class _Mixins(object):
    ...
    def setup_config(self, file):
        self.config = file.add_ini_file(file=file)
        self.strings_variable1 = self.config.add_string(
            name="strings.variable1", default="value1"
        )
        self.integer_variable11 = self.config.add_integer(
            name="integers.variable1", default=1
        )
        self.list_variable1 = self.config.add_list(
            name="lists.variable1", default=["foo", "bar", "baz"]
        )

    def get_goal(self, goal):
        return goals.create(
            goal, self.workspace, ConsoleFrontend(interactive=False), map=SerialMap
        )
    ...
    def test_destroy(self):
        self.assertRaises(errors.NothingChanged, self.call, "destroy")
    ...
    def test_retain_list(self):
        self.config.add_list(
            ...
            default=serializers.Expression(
                lambda ctx, value: [str(next(self.counter))]
            ),
            retain_default=True,
        )
    ...
    def test_retain_ips(self):
        net = self.config.add_ip_allocations(name="subnets", network="10.30.0.0/20")
        net.add_ip_allocation(name="app-a", size=25)
        net.add_ip_allocation(name="app-b", size=25)
        self.call("apply")
        self.assertRaises(errors.NothingChanged, self.call, "apply")
    ...
    def test_echo_string(self):
        self.workspace.add_echo(text=self.strings_variable1)
        self.call("apply")
...
class TestIpNetwork(_Mixins, unittest.TestCase):
    ...
    def test_ensure_allocator_state_preserved(self):
        ...
        net = self.config.add_ip_allocations(name="subnets", network=network)
        net.add_ip_allocation(name="app-a", size=25)
        net.add_ip_allocation(name="app-b", size=25)
