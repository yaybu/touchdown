...
class SlackNotification(resource.Resource):
    ...
    icon_url = argument.String(
        default="https://avatars3.githubusercontent.com/u/5338681", field="icon_url"
    )
    icon_emoji = argument.String(field="icon_emoji")
    text = argument.String(field="text")
    attachments = argument.ResourceList(
        Attachment,
        field="attachments",
        serializer=serializers.List(serializers.Resource(), skip_empty=True),
    )
...
class SlackNotificationAction(action.Action):
    ...
    def run(self):
        response = requests.post(
            ...
            data=serializers.Json(serializers.Resource()).render(
