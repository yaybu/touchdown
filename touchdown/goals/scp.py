...
class Scp(Goal):
    ...
    def setup_argparse(cls, parser):
        parser.add_argument("source", metavar="SOURCE", type=str, help="What to copy")
        parser.add_argument(
            "destination", metavar="DESTINATION", type=str, help="Where to copy it"
        )
    ...
    def execute(self, source, destination):
        for path in (source, destination):
            ...
            raise errors.Error(
                "Either source or destination must contain a target server that touchdown knows about"
            )
