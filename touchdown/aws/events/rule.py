...
class EventTarget(Resource):
    ...
    function = argument.Resource(
        Function, field="Arn", serializer=serializers.Property("FunctionArn")
    )
...
class EventRule(Resource):
    ...
    enabled = argument.Boolean(
        field="State",
        serializer=serializers.Boolean(on_true="ENABLED", on_false="DISABLED"),
    )

    role = argument.Resource(
        Role, field="RoleArn", serializer=serializers.Property("Arn")
    )

    targets = argument.ResourceList(EventTarget)
...
class Describe(SimpleDescribe, Plan):
    ...
    def annotate_object(self, obj):
        response = self.client.list_targets_by_rule(Rule=self.resource.name)
