...
class StreamingLoggingConfig(Resource):
    ...
    bucket = argument.Resource(
        Bucket, field="Bucket", serializer=serializers.Default(default=None), default=""
    )
...
class StreamingDistribution(Resource):
    ...
    extra_serializers = {
        "CallerReference": serializers.Expression(
            lambda runner, object: runner.get_plan(object)
            .object.get("StreamingDistributionConfig", {})
            .get("CallerReference", str(uuid.uuid4()))
        ),
    ...
    origin_access_identity = argument.String(
        default="", field="OriginAccessIdentity", group="s3origin"
    )
...
class Describe(SimpleDescribe, Plan):
    ...
    def get_waiter(self, description, waiter, eventual_consistency_threshold=1):
        return StreamingDistributionWaiter(
    ...
    def annotate_object(self, obj):
        ...
        distribution = {"ETag": result["ETag"], "Id": obj["Id"]}
...
class Apply(SimpleApply, Describe):
    ...
    signature = (Present("name"), Present("bucket"))

    def get_create_serializer(self):
        return serializers.Dict(StreamingDistributionConfig=serializers.Resource())
    ...
    def destroy_object(self):
        if self.object["StreamingDistributionConfig"].get("Enabled", False):
            yield self.generic_action(
                ...
                StreamingDistributionConfig=serializers.Resource(Enabled=False),
            )
