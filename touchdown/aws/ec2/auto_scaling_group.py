...
class AutoScalingGroupTag(Resource):
    ...
    resource_type = argument.Serializer(
        serializer=serializers.Const("auto-scaling-group"), field="ResourceType"
    )
...
class AutoScalingGroup(Resource):
    ...
    launch_configuration = argument.Resource(
        LaunchConfiguration, field="LaunchConfigurationName"
    )
    ...
    subnets = argument.ResourceList(
        ...
        serializer=serializers.CommaSeperatedList(
            serializers.List(serializers.Identifier(), skip_empty=True)
        ),
    )

    load_balancers = argument.ResourceList(
        LoadBalancer, field="LoadBalancerNames", update=False
    )
    ...
    termination_policies = argument.List(
        default=lambda i: ["Default"], field="TerminationPolicies"
    )
    replacement_policy = argument.String(
        choices=["singleton", "graceful"], default="graceful"
    )
...
class WaitForHealthy(Action):
    ...
    def run(self):
        while True:
            ...
            if (
                len([i for i in asg["Instances"] if i["LifecycleState"] == "InService"])
                >= self.resource.min_size
            ):
                return True
            time.sleep(5)
...
class ReplaceInstances(Action):
    ...
    def __init__(self, plan, instance_ids):
        ...
        self.desired_capacity = plan.object.get(
            "DesiredCapacity", self.resource.min_size
        )
...
class Apply(SimpleApply, Describe):
    ...
    def update_tags(self):
        diff = serializers.Argument("tags").diff(
            self.runner, self.resource, self.object.get("Tags", [])
        )
    ...
    def update_object(self):
        ...
        launch_config_name = self.runner.get_plan(
            self.resource.launch_configuration
        ).resource_id
        instances = []
...
class TerminateASGInstances(Action):
    ...
    def run(self):
        ...
        while True:
            activities = self.plan.client.describe_scaling_activities(
                AutoScalingGroupName=self.resource.name
            )["Activities"]
            if (
                len(tuple(a for a in activities if a["StatusCode"] == "InProgress"))
                == 0
            ):
                break
            time.sleep(10)
...
class Instance(ssh.Instance):
    ...
    def get_instances(self, runner):
        ...
        if len(obj.get("Instances", [])) == 0:
            raise errors.ServiceNotReady(
                "No instances currently running in group {}".format(self.adapts)
            )

        asg_inservice = list(
            filter(
                lambda x: x["LifecycleState"] == "InService", obj.get("Instances", [])
            )
        )

        if len(asg_inservice) == 0:
            raise errors.ServiceNotReady(
                "None of the instances in {} are in service".format(self.adapts)
            )
        ...
        if len(instances) == 0:
            raise errors.ServiceNotReady(
                "No instances available in {}".format(self.adapts)
            )
