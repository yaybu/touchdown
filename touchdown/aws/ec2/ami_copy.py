...
class ImageCopy(Resource):
    ...
    extra_serializers = {
        "SourceRegion": serializers.Expression(
            lambda runner, obj: obj.source.account.region
        )
    }

    def __str__(self):
        return 'image_copy "{}" (copy from {} to {})'.format(
            self.name, self.source.account.region, self.account.region
        )
