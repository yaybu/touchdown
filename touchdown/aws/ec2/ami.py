...
class BuildInstance(Action):
    ...
    def description(self):
        yield 'Build new AMI "{}" from "{}"'.format(
            self.resource.name, self.resource.source_ami
        )
    ...
    def destroy_security_group(self, security_group):
        ...
        self.plan.client.delete_security_group(GroupId=security_group["GroupId"])
    ...
    def destroy_keypair(self, keypair):
        ...
        self.plan.client.delete_key_pair(KeyName=keypair["KeyName"])

    def create_instance(self, keypair, security_group):
        self.plan.echo(
            "Creating a source instance from {}".format(self.resource.source_ami)
        )
        ...
        self.plan.echo(
            "Waiting for instance {} to boot...".format(instance["InstanceId"])
        )
        self.plan.client.get_waiter("instance_running").wait(
    ...
    def deploy_instance(self, keypair, instance):
        ...
        cli.run_script(
            **serializers.Resource().render(self.runner, self.resource.provisioner)
        )

    def terminate_instance(self, instance):
        self.plan.echo("Terminating instance")
        self.plan.client.terminate_instances(InstanceIds=[instance["InstanceId"]])

        self.plan.echo("Waiting for instance to go away")
        self.plan.client.get_waiter("instance_terminated").wait(
            InstanceIds=[instance["InstanceId"]]
        )
    ...
    def run(self):
        ...
        with self.stack:
            ...
            image = self.plan.client.create_image(
                Name=self.resource.name, InstanceId=instance["InstanceId"]
            )

            self.plan.echo("Waiting for image to become available")
            self.plan.client.get_waiter("image_available").wait(
                ImageIds=[image["ImageId"]]
            )

        self.plan.object = {self.plan.key: image[self.plan.key]}
    ...
    def update_object(self):
        ...
        if self.object:
            results = self.client.describe_image_attribute(
                ImageId=self.object["ImageId"], Attribute="launchPermission"
        ...
        if add or remove:
            yield self.generic_action(
                ...
                LaunchPermission=dict(Add=add, Remove=remove),
            )
