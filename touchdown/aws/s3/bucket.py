...
class Bucket(Resource):
    ...
    arn = argument.Callable(lambda instance: "arn:aws:s3:::{}".format(instance.name))
        ...
        for section in value.split("."):
            ...
            if not re.search(r"^([a-z0-9]{1})([a-z0-9\-]*[a-z0-9])$", value):
                raise InvalidParameter(
                    "Value must start and end with a number or letter and can only contain numbers, letters and hyphens"
                )
        return value
    ...
    region = argument.String(
        ...
        serializer=serializers.Dict(LocationConstraint=serializers.Identity()),
    )
...
class Apply(SimpleApply, Describe):
    ...
    def update_accelerate_config(self):
        if self.object.get("Status", "Suspended") != self.resource.accelerate:
            yield self.generic_action(
                ...
                Bucket=self.resource.name,
                AccelerateConfiguration={"Status": self.resource.accelerate},
            )

    def update_notification_config(self):
        local_s = serializers.Default(
            serializers.Resource(group="notifications"), serializers.Const({})
        )
        d = serializers.Resource(group="notifications").diff(
            self.runner, self.resource, self.object.get("NotificationConfiguration", {})
        )
    ...
    def update_object(self):
        ...
        if not self.object and self.resource.rules:
            ...
            local = [
                serializers.Resource().render(self.runner, rule)
                for rule in self.resource.rules
            ]
...
class Destroy(SimpleDestroy, Describe):
    ...
    def get_destroy_serializer(self):
        return serializers.Dict(Bucket=self.resource.name)
