...
class Record(Resource):
    ...
    values = argument.List(
        ...
        serializer=serializers.List(
            serializers.Dict(Value=serializers.Identity()), skip_empty=True
        ),
    )
    ...
    alias = argument.Resource(
        AliasTarget, field="AliasTarget", serializer=serializers.Resource()
    )
    ...
    def update_object(self):
        ...
        for local in self.resource.records:
            for remote in remote_records:
                ...
                description.append(
                    "Name => {}, Type={}, Action=UPSERT".format(local.name, local.type)
                )
        ...
        if not self.resource.shared:
            for remote in remote_records:
                for local in self.resource.records:
                    ...
                    changes.append(
                        serializers.Const(
                            {"Action": "DELETE", "ResourceRecordSet": remote}
                        )
                    )
                    description.append(
