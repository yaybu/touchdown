...
class Rule(Resource):
    ...
    protocol = argument.String(
        default="tcp", choices=["tcp", "udp", "icmp"], field="Protocol"
    )
    port = argument.Resource(
        PortRange, field="PortRange", serializer=serializers.Resource()
    )
    icmp = argument.Resource(
        IcmpTypeCode, field="IcmpTypeCode", serializer=serializers.Resource()
    )
    action = argument.String(
        default="allow", choices=["allow", "deny"], field="RuleAction"
    )
...
class Describe(ReplacementDescribe, Plan):
    ...
    def get_describe_filters(self):
        ...
        return {"Filters": [{"Name": "vpc-id", "Values": [vpc.resource_id]}]}
    ...
    def _insert_rule(self, rule, rule_number, egress):
        return self.generic_action(
            "Add {direction} rule: {rule}".format(
                rule=rule, direction="egress" if egress else "ingress"
            ),
