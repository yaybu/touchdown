...
class Rule(Resource):
    ...
    protocol = argument.String(
        default="tcp", choices=["tcp", "udp", "icmp"], field="IpProtocol"
    )
    port = argument.Integer(min=-1, max=32768)
    from_port = argument.Integer(
        default=lambda r: r.port, min=-1, max=32768, field="FromPort"
    )
    to_port = argument.Integer(
        default=lambda r: r.port, min=-1, max=32768, field="ToPort"
    )
    ...
    security_group = argument.Resource(
        ...
        serializer=serializers.ListOfOne(
            serializers.Dict(
                UserId=serializers.Property("OwnerId"), GroupId=serializers.Identifier()
            )
        ),
    )
    ...
    network = argument.IPNetwork(
        field="IpRanges",
        serializer=serializers.ListOfOne(serializers.Dict(CidrIp=serializers.String())),
    )
        ...
        if self.security_group:
            ...
            if sg and sg.object:
                for group in rule.get("UserIdGroupPairs", []):
                    if (
                        group["GroupId"] == sg.resource_id
                        and group["UserId"] == sg.object["OwnerId"]
                    ):
                        return True
        return False
    ...
    def matches(self, runner, rule):
        ...
        if self.to_port != rule.get("ToPort", None):
            return False
        return self.match_security_group(runner, rule) or self.match_network(
            runner, rule
        )
...
class SecurityGroup(Resource):
    ...
    egress = argument.ResourceList(
        Rule, default=lambda instance: [dict(protocol=-1, network=["0.0.0.0/0"])]
    )
...
class Apply(TagsMixin, SimpleApply, Describe):
    ...
    signature = (Present("name"), Present("description"))
            ...
            for remote_rule in self.object.get("IpPermissions", []):
                ...
                yield self.generic_action(
                    ...
                    IpPermissions=serializers.ListOfOne(
                        local_rule.serializer_with_kwargs()
                    ),
                )
