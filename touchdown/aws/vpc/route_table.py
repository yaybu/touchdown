...
class Route(Resource):
    ...
    nat_gateway = argument.Resource(
        "touchdown.aws.vpc.nat_gateway.NatGateway", field="NatGatewayId"
    )
...
class Apply(TagsMixin, SimpleApply, Describe):
    ...
    def update_routes(self):
        ...
        remote_routes = list(
            d
            for d in self.object.get("Routes", [])
            if d.get("GatewayId", "") != "local"
        )
        remote_routes = list(
            d for d in remote_routes if d["Origin"] != "EnableVgwRoutePropagation"
        )
