...
class Database(Resource):
    ...
    security_groups = argument.ResourceList(
        SecurityGroup, field="VpcSecurityGroupIds", update=False
    )
    ...
    subnet_group = argument.Resource(
        SubnetGroup, field="DBSubnetGroupName", update=False
    )
    ...
    storage_type = argument.String(
        field="StorageType", choices=["standard", "gp2", "io1"]
    )
    ...
    def clean_storage_encrypted(self, value):
        ...
        if self.instance_class not in supported_instances:
            raise InvalidParameter(
                "Encryption-at-rest is only supported for a subset of instance classes (such as db.m3.medium)"
            )
        return True
...
class Destroy(SimpleDestroy, Describe):
    ...
    def get_destroy_serializer(self):
        return serializers.Dict(
            DBInstanceIdentifier=self.resource_id, SkipFinalSnapshot=True
        )
