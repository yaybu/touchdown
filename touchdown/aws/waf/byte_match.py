...
class ByteMatchTuple(Resource):
    ...
    header = argument.String(field="Data", group="FieldToMatch")

    _field_to_match = argument.Serializer(
        field="FieldToMatch", serializer=serializers.Resource(group="FieldToMatch")
    )
    ...
    position = argument.String(
        ...
        default="EXACTLY",
        choices=["CONTAINS", "CONTAINS_WORD", "EXACTLY", "STARTS_WITH", "ENDS_WITH"],
    )

    target = argument.String(field="TargetString", min=1, max=50)
