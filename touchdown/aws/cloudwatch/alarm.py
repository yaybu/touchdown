...
class Alarm(Resource):
    ...
    ok_actions = argument.ResourceList(
        AlarmDestination,
        field="OKActions",
        serializer=serializers.List(serializers.Resource()),
    )
    alarm_actions = argument.ResourceList(
        AlarmDestination,
        field="AlarmActions",
        serializer=serializers.List(serializers.Resource()),
    )
    ...
    dimensions = argument.ResourceList(
        Dimension,
        max=10,
        field="Dimensions",
        serializer=serializers.List(serializers.Resource()),
    ...
    statistic = argument.String(
        choice=["SampleCount", "Average", "Sum", "Minimum", "Maximum"],
        field="Statistic",
    )
    ...
    comparison_operator = argument.String(
        choices=[
...
class Describe(SimpleDescribe, Plan):
    ...
    def get_describe_filters(self):
        return {"AlarmNames": [self.resource.name]}
...
class Destroy(SimpleDestroy, Describe):
    ...
    def get_destroy_serializer(self):
        return serializers.Dict(AlarmNames=[self.resource.name])
