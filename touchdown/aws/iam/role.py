...
class Role(Resource):
    ...
    assume_role_policy = argument.Dict(
        field="AssumeRolePolicyDocument", serializer=serializers.Json(serializers.Map())
    )
...
class Apply(SimpleApply, Describe):
    ...
    def update_assume_role_policy(self):
        ...
        diff = pd.diff(
            self.runner,
            self.resource,
            json.dumps(self.object.get("AssumeRolePolicyDocument", {})),
        )
        if diff.matches():
            return
    ...
    def update_object(self):
        ...
        if self.object:
            policy_names = self.client.list_role_policies(RoleName=self.resource.name)[
                "PolicyNames"
            ]
        ...
        for name, document in self.resource.policies.items():
            ...
            if not changed:
                policy = self.client.get_role_policy(
                    RoleName=self.resource.name, PolicyName=name
                )
...
class GetCredentialsPlan(Describe):
    ...
    def sts(self):
        if not self._sts:
            self._sts = self.runner.get_plan(
    ...
    def get_credentials(self):
        ...
        creds = self.sts.assume_role(
            RoleArn=role["Arn"], RoleSessionName="touchdown-get-credentials"
        )["Credentials"]
...
class Plan(Describe):
    ...
    def get_signin_url(self):
        ...
        creds = self.sts.assume_role(
            RoleArn=role["Arn"], RoleSessionName="touchdown-get-signin-url"
        )["Credentials"]
