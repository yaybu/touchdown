...
class FunctionSerializer(serializers.Serializer):
    ...
    def mkinfo(self, name):
        info = zipfile.ZipInfo(name, date_time=(2004, 7, 6, 0, 0, 0))
...
class Function(Resource):
    ...
    runtime = argument.String(
        field="Runtime", default="python2.7", choices=["nodejs", "java8", "python2.7"]
    )
    ...
    code = argument.Bytes(
        field="Code",
        update=False,
        serializer=serializers.Dict(ZipFile=serializers.Bytes()),
    )
    ...
    security_groups = argument.ResourceList(
        SecurityGroup, field="SecurityGroupIds", group="vpc_config"
    )
    subnets = argument.ResourceList(Subnet, field="SubnetIds", group="vpc_config")
    vpc_config = argument.Serializer(
        serializer=serializers.Resource(group="vpc_config"), field="VpcConfig"
    )
...
class Apply(SimpleApply, Describe):
    ...
    signature = (
        ...
        # Present('runtime'),
        XOR(Present("code"), Present("s3_file")),
    )
            ...
            yield Skip(
                self.generic_action(
                    [
                        ...
                        "{} => {}".format(self.object["CodeSha256"], digest),
                    ],
