...
class Certificate(Resource):
    ...
    alternate_names = argument.List(argument.String(), field="SubjectAlternativeNames")
    validation_options = argument.ResourceList(
        DomainValidationOption, field="DomainValidationOptions"
    )
...
class CertificateWaiter(Waiter):
    def get_waiter_filters(self):
        return {"CertificateArn": self.plan.resource_id}
...
class Destroy(SimpleDestroy, Describe):
    ...
    def get_destroy_serializer(self):
        return serializers.Dict(CertificateArn=self.resource.arn)
