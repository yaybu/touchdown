...
class Queue(Resource):
    ...
    delay_seconds = argument.Integer(
        min=0,
        max=900,
        field="DelaySeconds",
    ...
    def annotate_object(self, queue):
        queue.update(
            self.client.get_queue_attributes(
                QueueUrl=queue["QueueUrl"], AttributeNames=["All"]
            )["Attributes"]
        )
        return queue
...
class Apply(SimpleApply, Describe):
    ...
    def update_object(self):
        d = serializers.Resource(group="attributes").diff(
            self.runner, self.resource, self.object
        )
        if not d.matches():
...
class Destroy(SimpleDestroy, Describe):
    ...
    def get_destroy_serializer(self):
        return serializers.Dict(QueueUrl=serializers.Identifier())
