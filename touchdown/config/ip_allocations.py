...
class Describe(plan.Plan):
    ...
    def get_actions(self):
        ...
        self.runner.get_service(self.resource, "ip_allocator").load(
            self.network(), self.object
        )
        return []
...
class IpAllocator(plan.Plan):
    ...
    def allocate(self, name, prefixlen):
        ...
        if prefixlen < int(network.prefixlen):
            raise ValueError(
                "Cannot fit /{} inside /{}".format(prefixlen, network.prefixlen)
            )
        ...
        with self.allocation_lock:
            for i in range(prefixlen, int(network.prefixlen) - 1, -1):
                ...
                raise ValueError(
                    "There is not enough space left to allocate a /{}".format(prefixlen)
                )
